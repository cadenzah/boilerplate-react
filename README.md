...
This means your web applications's static assets - the bundled `.js`/`.css` files or images will be served under the URL above which contains `/<REPOSITORY_NAME>` at back. Usually the URL slug after the first `/` have to be considered as a top-level route path, but that is not the case here. It is a part of the public URL.
